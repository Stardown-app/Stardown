...
const wantsStableOrPrereleaseEl = document.querySelector(
    "#wantsStableOrPrerelease",
);
...
const installedWithTerminalEl = document.querySelector(
    "#installedWithTerminal",
);
...
const willInstallWithTerminalEl = document.querySelector(
    "#willInstallWithTerminal",
);
...
firefoxEl.checked =
    typeof InstallTrigger !== "undefined" && !chromiumEl.checked;
safariEl.checked =
    navigator.vendor &&
...
installedWithStoreEl.addEventListener(
    "change",
    buildAndShowInstructionsAndScroll,
);
installedWithZipEl.addEventListener(
    "change",
    buildAndShowInstructionsAndScroll,
);
installedWithTerminalEl.addEventListener(
    "change",
    buildAndShowInstructionsAndScroll,
);
willInstallWithStoreEl.addEventListener(
    "change",
    buildAndShowInstructionsAndScroll,
);
willInstallWithZipEl.addEventListener(
    "change",
    buildAndShowInstructionsAndScroll,
);
willInstallWithTerminalEl.addEventListener(
    "change",
    buildAndShowInstructionsAndScroll,
);
...
const [stableMajor, stableMinor, stablePatch] = latestStableVersion
    .substring(1)
    .split(".")
    .map((n) => parseInt(n));
const [preMajor, preMinor, prePatch] = latestPrereleaseVersion
    .split("-")[0]
    .substring(1)
    .split(".")
    .map((n) => parseInt(n));
...
async function main() {
    ...
    const isUpdating =
        new URLSearchParams(window.location.search).get("updating") === "true";
    if (isUpdating) {
...
function installWithZip(instructions) {
    if (
        !isStableNewer &&
        !wantsStableEl.checked &&
        !wantsPrereleaseEl.checked
    ) {
        return;
    }
...
function showInstructions(instructions) {
    ...
    if (instructions.steps.length > 0) {
        html.push(
            "<ol><li>" + instructions.steps.join("</li><li>") + "</li></ol>",
        );
    }
